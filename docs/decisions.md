...
| D-201 | §10.10 (release strategy; supersedes the docs-only-release version convention practiced by `v1.1.1` and stated in its CHANGELOG entry) | Locked | **Every release bumps the in-proc version — documentation-only releases included.** Each tagged release, whatever it contains, updates `@ToolVersion`, `Build-Date-Utc` (header and `@BuildDateUtc`), so `About`, `Help`, `Status`, and the Markdown report header always identify the release actually installed. The rule is enforced structurally, not by memory: `scripts/check-version-tag.sh` fails any tagged build whose artifact `Tool-Version` header does not match the tag, wired into `release.yml`'s gate and the `ci-tier1` static-analysis job (no-op on untagged builds). `SchemaVersion` (D-038, tracks DDL) and `RulePackVersion` (D-085, tracks rule changes) are unaffected — they version different things and keep their own rules. Historical exception, recorded rather than rewritten: `v1.1.1` shipped reporting `ToolVersion 1.1.0` under the prior convention; `v1.1.2` is the first release under this one. | Two artifacts that behave differently in no way but still cannot be told apart from inside SQL Server is exactly the ambiguity `About` exists to prevent. The prior convention ("docs-only releases leave the artifact untouched") also made `release.yml`'s existing tag-vs-header gate fail on any docs-only tag — the convention and the pipeline contradicted each other, and the pipeline was right. | Docs-only releases now change the artifact's bytes and checksum, so no release is ever byte-identical to its predecessor; anyone comparing checksums across docs-only releases must expect the version-metadata diff. Version bumps become one more mechanical step per release — mitigated by the gate, which turns a forgotten bump into a failed build instead of a shipped inconsistency. |

---

## Summary

- **Total decisions:** 201 (D-001 through D-201)
- **Locked:** 194
- **Tentative:** 1 (D-181)
