...
| Azure SQL Database (or any external scheduler) grows the repository even though Collect runs fine | The external schedule runs `Collect` only — there is no local Agent to purge for you, and Collect alone is not sufficient. | Add a daily external job for `EXEC dbo.sp_SQLFlightRecorder @Mode=N'Purge', @WhatIf=0;` (Elastic Jobs, Automation, cron — see [scheduling.md](scheduling.md)). Result set 7's `OldestSnapshotVsRetention`/`PurgeKeepingUp` warnings confirm when it is missing. |
| `Configure` returns `InvalidConfigValue` for a retention key | The value is outside the guardrails: `SnapshotRetentionDays` allows 1–31, `RunLogRetentionDays` 1–124 (D-199); `FR_Config` was not changed. | Pick a value in range. SQLFR is an operational diagnostic recorder, not a long-term warehouse — export data you must keep longer. Pre-guardrail out-of-range values already stored are flagged by result set 7 (`RetentionConfiguration=Warning`). |
